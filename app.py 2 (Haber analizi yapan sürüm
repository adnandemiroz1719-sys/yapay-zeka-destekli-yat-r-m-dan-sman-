...
# -----------------------------------------------------------------
st.set_page_config(layout="wide", page_title="Yapay Zeka Destekli Finansal Analiz ve Yatırım Danışmanlığı Uygulaması")

# BURAYA FINNHUB API ANAHTARINI YAZ (finnhub.io'dan ücretsiz alabilirsin)
# Eğer anahtarın yoksa boş bırakabilirsin, sistem çalışır ama haber çekmez.
FINNHUB_API_KEY = "d66qqvpr01qnh6sfuvpgd66qqvpr01qnh6sfuvq0"
...
# -----------------------------------------------------------------
# 2.5. DUYGU ANALİZİ MOTORU (PROFESYONEL MODÜL - YENİ)
# -----------------------------------------------------------------
@st.cache_resource
def load_finbert():
    # Finansal metinler için eğitilmiş özel model: FinBERT-Tone
    # İlk çalıştırmada modeli indirir (yaklaşık 400MB)
...
class SentimentEngine:
    ...
    def get_sentiment(self, ticker):
        if not self.model_loaded:
            return 50, ["Model Yüklenemedi"]

        try:
            # --- AKILLI EŞLEŞTİRME (MAPPING) ---
            # Endeks sembolleri yerine haber akışı yoğun olan ETF'leri kullanıyoruz.
            ticker_map = {
                "^GSPC": "SPY",  # S&P 500 -> SPY ETF
                "^NDX": "QQQ",  # Nasdaq 100 -> QQQ ETF
                "^GDAXI": "EWG",  # DAX -> Germany ETF
                "XU100.IS": "TUR",  # BIST 100 -> Turkey ETF
                "^N225": "EWJ",  # Nikkei -> Japan ETF
                "000001.SS": "MCHI"  # Shangai -> China ETF
            }

            # Eğer listede varsa ETF kodunu al, yoksa normal kodu kullan
            search_ticker = ticker_map.get(ticker, ticker)

            # Temizlik (Kalan semboller için)
            clean_ticker = search_ticker.replace("^", "").split(".")[0]
            if "BTC" in ticker: clean_ticker = "BTC"
            ...
            url = f"https://finnhub.io/api/v1/company-news?symbol={clean_ticker}&from={start_date}&to={today}&token={self.api_key}"
            response = requests.get(url, timeout=5)
            data = response.json()

            if isinstance(data, dict) and 'error' in data:
                return 50, [f"⚠️ API Limit/Hata: {data['error']}"]

            if not data or not isinstance(data, list):
                return 50, [f"⚠️ {search_ticker} için güncel haber bulunamadı."]

            headlines = [item['headline'] for item in data[:5]]
            if not headlines:
                return 50, ["Güncel haber bulunamadı."]
            ...
            for i, res in enumerate(results):
                ...
                score_sum += val
                details.append(f"{icon} {headlines[i]} ({label})")

            avg_score = score_sum / len(results)
            return avg_score, details

        except Exception as e:
            return 50, [f"Hata: {str(e)}"]
...
def calculate_match_score(ticker, user_prefs, market_data):
    ...
    asset_risk = info.get('risk_level', 5)
    asset_fund = info.get('fund_score', 5)

    tech_score = market_data['tech']['Score']
    ml_return = market_data['ml_return']
    # YENİ: Duygu Puanı
    sent_score = market_data['sentiment']['score']
    ...
    if abs(risk_diff) <= 3:
        ...
        score -= 10
        reasons.append("⚠️ Risk profili uyumsuz")

    # 2. TEKNİK & TEMEL
    score += (tech_score * (u_tech / 10)) * 0.25
    score += (asset_fund * 10 * (u_fund / 10)) * 0.20

    # 3. YENİ: DUYGU ANALİZİ ETKİSİ (%20 Etki)
    if sent_score > 60:
        score += 20
        reasons.append(f"📰 Haberler Pozitif (Duygu Skoru: {sent_score:.0f})")
    elif sent_score < 40:
        score -= 20
        reasons.append(f"📰 Haberler Negatif (Duygu Skoru: {sent_score:.0f})")
    else:
        score += 5  # Nötr iyidir
        reasons.append("📰 Haber Akışı Nötr/Dengeli")
...
st.title("🧠 Yapay Zeka Destekli Finansal Analiz ve Yatırım Danışmanlığı Uygulaması")
st.markdown("*(Risk, Teknik, Temel ve **Duygu Analizi (Sentiment)** verilerini işler)*")
...
if btn_run:
    if data is None:
        ...
        # --- YENİ: Motoru Başlat ---
        sentiment_engine = SentimentEngine(api_key=FINNHUB_API_KEY)
        # ---------------------------

        scored_assets = []
        seen_tickers = set()

        with st.spinner('Piyasa verileri, teknik analiz ve haberler taranıyor...'):
            for ticker in GLOBAL_ASSETS:
                ...
                try:
                    ...
                    # --- YENİ: Haber Analizi Yap ---
                    s_score, s_details = sentiment_engine.get_sentiment(ticker)
                    # -------------------------------
                    ...
                    m_data = {
                        ...
                        "ml_return": ml_ret,
                        "status": status,
                        "sentiment": {"score": s_score, "details": s_details}  # YENİ
                    }
            ...
            with st.expander(f"{color} #{i + 1} {info['name']} ({asset['Kod']}) | Puan: {scr}"):
                ...
                with tab1:
                    ...
                    with col_info:
                        ...
                        # YENİ: Haber Detayları
                        st.divider()
                        st.markdown("**📰 YZ Haber Duyarlılık Analizi (FinBERT):**")
                        if asset['Duygu_Detay']:
                            for d in asset['Duygu_Detay']:
                                st.caption(d)
                        else:
                            st.caption("Veri çekilemedi veya haber yok.")
